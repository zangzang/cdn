...
이 문서는 .NET 9 SDK를 사용한 빌드 및 배포를 위한 Publish Profile(.pubxml) 설정 방법을 설명합니다. Publish Profile은 `Properties\PublishProfiles` 폴더에 저장되며, `dotnet publish` 명령어로 사용됩니다. 아래는 모든 주요 속성을 포함한 포괄적인 예제와 사용 방법, 그리고 `RuntimeIdentifier` 지정이 성능에 미치는 영향을 포함합니다. 추가로 Jenkins Pipeline 예제(심플 버전과 디테일 버전)를 포함합니다.
...
- **Jenkins 플러그인**: .NET SDK Support 플러그인을 설치하면 `dotnetBuild`, `dotnetPublish` 등의 스텝을 사용할 수 있습니다.