...
이 문서는 .NET 9 SDK를 사용한 빌드 및 배포를 위한 Publish Profile(.pubxml) 설정 방법을 설명합니다. Publish Profile은 `Properties\PublishProfiles` 폴더에 저장되며, `dotnet publish` 명령어로 사용됩니다. 또한, Jenkins의 Generic Webhook Trigger Plugin을 사용한 CI/CD 파이프라인 트리거 예제(심플 버전 및 디테일 버전)를 포함합니다.

## Publish Profile 예제 (Production.pubxml)

다음은 ASP.NET Core 웹 애플리케이션(또는 콘솔 앱)을 위한 완전한 `.pubxml` 파일 예제입니다. 파일 시스템 배포를 기본으로 하며, .NET 9의 새로운 기능(예: TieredPGO, AppHostDotNetSearch)과 컨테이너 지원을 포함합니다.
...
Jenkins Pipeline은 CI/CD를 자동화하기 위해 사용됩니다. 아래는 .NET 9 프로젝트를 위한 Groovy 기반 Jenkinsfile 예제입니다. Jenkins 에이전트에 .NET 9 SDK가 설치되어 있어야 합니다.

### 심플 버전 (Jenkinsfile)
이 버전은 기본적으로 소스 체크아웃, 복원, 빌드, 퍼블리시를 수행합니다.
...
pipeline {
    ...
    stages {
        ...
        stage('Publish') {
            steps {
                sh 'dotnet publish --configuration Release /p:PublishProfile=Production'
            }
        }
    }
}
...
### 디테일 버전 (Jenkinsfile)
이 버전은 테스트, 아티팩트 저장, 에러 처리, 환경 변수 설정을 추가합니다. 또한 Self-contained 배포를 위한 RID를 지정합니다.

```groovy
pipeline {
    agent any
    environment {
        DOTNET_CLI_TELEMETRY_OPTOUT = '1'  // 텔레메트리 비활성화
    ...
    stages {
        ...
        stage('Publish') {
            steps {
                sh 'dotnet publish --configuration Release --no-build /p:PublishProfile=Production -r win-x64'
            }
        }
...
Generic Webhook Trigger Plugin은 GitHub, Bitbucket 등의 웹훅을 받아 Pipeline을 트리거합니다. 아래는 .NET 9 프로젝트에 맞춘 트리거 설정 예제입니다.
...
### 심플한 방법: 토큰만 사용
토큰 인증만으로 Pipeline을 트리거합니다. 페이로드 파싱 없이 모든 웹훅 요청을 처리하므로 간단한 CI 트리거에 적합합니다.
...
#### 사용 예시
- **GitHub 설정**: Webhooks > Payload URL에 위 URL 입력, Content type: `application/json`.
- **트리거**: 모든 푸시/PR 이벤트에서 토큰이 맞으면 빌드 시작.
- **장점**: 설정 간단, 빠른 트리거.
- **단점**: 조건 필터링 없어 불필요한 빌드 가능.

### 디테일한 방법: 토큰 + JSONPath Expression
토큰 인증 후 JSON 페이로드에서 변수를 추출하고, 정규식 필터로 특정 브랜치(예: `main`, `develop`)에서만 트리거합니다.

#### Job 설정
1. Jenkins Job > Configure > **Build Triggers** > "Generic Webhook Trigger" 체크.
2. **Token**: `my-detailed-token` 입력.
3. **JSONPath Expressions**:
   - Variable: `branch` | Expression: `$.ref` (e.g., "refs/heads/main").
   - Variable: `commit_sha` | Expression: `$.after` (커밋 SHA).
4. **Optional Filter**:
   - Text: `${branch}`
    ...
    stages {
        ...
        stage('Build') {
            ...
            steps {
                ...
                sh 'dotnet build --configuration ${BUILD_ENV == "production" ? "Release" : "Debug"}'
            }
        }
        stage('Test') {
            steps {
        ...
        stage('Security Scan') {
            ...
            steps {
                ...
                sh 'dotnet sonarscanner begin /k:"MyProject" /d:sonar.host.url="http://sonar-url"'
        ...
        stage('Deploy') {
            steps {
                ...
                sh 'dotnet publish --configuration Release /p:PublishProfile=Production'
                archiveArtifacts artifacts: 'bin/Release/net9.0/publish/**', fingerprint: true
            }
        }
    }
    post {
...
- **장점**: 조건 필터링으로 효율적, 변수로 동적 로직 구현.
- **단점**: JSONPath 설정 복잡.

## 추가 참고
- **TieredPGO**: .NET 9에서 동적 PGO로 성능 10-20% 향상.
- **AppHostDotNetSearch**: .NET 9 런타임 검색 경로 설정.
- **Jenkins 플러그인**: Generic Webhook Trigger, MSTest, Email Extension 설치 권장.
